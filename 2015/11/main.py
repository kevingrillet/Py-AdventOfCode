...
def process(inpt: str) -> str:
    ...
    while True:
        ...
        if regex_mistaken.search(password_new) or len(regex_double.findall(password_new)) < 2:
            continue

        straight = False
        pos = 0
        while pos < len(password_new) - 2:
            if password_new[pos] not in ('y', 'z') \
                    and password_new[pos + 1] == alphabet[alphabet.find(password_new[pos]) + 1] \
                    and password_new[pos + 2] == alphabet[alphabet.find(password_new[pos]) + 2]:
                straight = True
                break
            pos += 1
