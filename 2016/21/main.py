...
def process_move(pwd: list[str], splt: list[str]) -> None:
    """Process move instruction."""
    pwd.insert(int(splt[-1]), pwd.pop(int(splt[2])))


def process_swap(pwd: list[str], splt: list[str]) -> None:
...
def process_rotate(pwd: list[str], splt: list[str], direction: int) -> None:
...
def process_reverse(pwd: list[str], splt: list[str], direction: int) -> None:
    """Process reverse instruction."""
...
def run(inpt: list[str], password: str, direction: int = 1) -> str:
    ...
    for line in inpt[::direction]:
        ...
        if line.startswith("move"):
            process_move(pwd, splt)
        elif line.startswith("swap"):
            process_swap(pwd, splt)
        elif line.startswith("rotate"):
            process_rotate(pwd, splt, direction)
        elif line.startswith("reverse"):
            process_reverse(pwd, splt, direction)
